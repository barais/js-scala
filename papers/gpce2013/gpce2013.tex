...
\code{SelectorFind} node (translated into a call to the general \code{querySelector} function) is
returned. In the latter case, the selector matches an ID selector so a
\code{DocumentGetElementById} node (translated into a call to the specialized \code{getElementById}
function).
...
code than others (this result is almost due to the special \code{for} notation, that has no
